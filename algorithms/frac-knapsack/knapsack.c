...
Pair *
solve_knapsack(unsigned int n_items, double *profits, double *weights, double max_weight)
{
...
main()
{
    ...
    for (int i = 0; i < n_items; ++i) scanf("%lf", profits + i);
    printf("Enter weights: ");
    for (int i = 0; i < n_items; ++i) scanf("%lf", weights + i);
    Pair * answer = solve_knapsack(n_items, profits, weights, 16);
    printf("Answer vector is: ");
    for (int i = 0; i < 6; ++i) printf("%.2f ", answer->fractions[i]);
    printf("\nProfit is: %.2f\n", answer->profit);
    return 0;
}
