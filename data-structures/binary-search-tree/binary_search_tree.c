...
Node *
insert(Node **root_ref, int value)
{
	if (*root_ref == NULL) {
...
Node *delete (Node **root_ref, int value)
{
	if (*root_ref == NULL) {
		...
		// CASE 3: two children
			...
			(**root_ref).right =
				delete (&((**root_ref).right), temp->value);
		}
	}
	return *root_ref;
}
...
int
count_internal_nodes(Node *root)
{
	if (root == NULL) {
		return 0;
...
int
count_external_nodes(Node *root)
{
	if (root == NULL) {
		return 0;
	} else if (root->left == NULL && root->right == NULL) {
