...
void
enqueue(QueueNode **head_ref, QueueNode **tail_ref, int value)
{
...
void
dequeue(QueueNode **head_ref, QueueNode **tail_ref)
{
	if (*head_ref == NULL && *tail_ref == NULL) {
...
BSTNode *
insert_BST(BSTNode **root_ref, int value)
{
	if (*root_ref == NULL) {
...
void
__reverse_path(BSTNode **root_ref, int value, QueueNode **head_ref,
	       QueueNode **tail_ref)
{
	if (*root_ref) {
		if (value < (*root_ref)->value) {
			enqueue(head_ref, tail_ref, (*root_ref)->value);
			__reverse_path(&((*root_ref)->left), value, head_ref,
				       tail_ref);
			(*root_ref)->value = queue_front(*head_ref);
			dequeue(head_ref, tail_ref);
		} else if (value > (*root_ref)->value) {
			enqueue(head_ref, tail_ref, (*root_ref)->value);
			__reverse_path(&((*root_ref)->right), value, head_ref,
				       tail_ref);
			(*root_ref)->value = queue_front(*head_ref);
			dequeue(head_ref, tail_ref);
		} else {
...
void
reverse_path(BSTNode **root_ref, int value)
{
...
void
inorder_traversal(BSTNode *node)
{
	if (node != NULL) {
...
void
preorder_traversal(BSTNode *node)
{
	if (node != NULL) {
		printf("%d ", node->value);
		inorder_traversal(node->left);
...
void
postorder_traversal(BSTNode *node)
{
	if (node != NULL) {
		inorder_traversal(node->left);
		inorder_traversal(node->right);
