...
void enqueue_rear(Node** head_ref, Node** tail_ref, int value) {
...
void enqueue_front(Node** head_ref, Node** tail_ref, int value) {
    Node* node = create_node(value);
    if (*head_ref == NULL && *tail_ref == NULL) {
        *head_ref = *tail_ref = node;
    } else {
...
void dequeue_front(Node** head_ref, Node** tail_ref) {
...
void dequeue_rear(Node** head_ref, Node** tail_ref) {
...
void traverse(Node** head_ref, Node** tail_ref) {
    printf("QUEUE: ");
    Node* iter = *head_ref;
    while (iter != NULL) {
        printf("%d ", iter->value);
